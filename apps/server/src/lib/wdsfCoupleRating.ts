...
const WEIGHTS = {
  avgPlace: 0.3,      // STRONG (35%) — База. Как высоко ты забираешься по турам.
  tier: 0.2,          // STRONG (25%) — Контекст. Победа на Grand Slam стоит дороже победы на локалке.
  trend: 0.05,         // MEDIUM (15%) — Форма. Растешь ты или падаешь по сравнению с прошлым месяцем.
  upsets: 0.14,        // HYPE (10%) — Апсеты. Наказал пару, которая выше тебя в мировом рейтинге? Получи буст!
  finalsPodium: 0.01,  // CLUTCH (10%) — Доминация. Умение доходить до медалей.
  worldRank: 0.3      // LOW (5%) — Якорь. Просто легкая поправка на глобальный статус пары.
} as const;
